    ...
    <div className="h-screen w-screen bg-slate-50 text-slate-900 font-sans karya-dashboard-theme overflow-hidden flex flex-col">
      ...
      {activeTab === 'overview' && (
        <main className="flex-1 overflow-y-auto bg-slate-50 p-4 lg:p-8 space-y-8 custom-scrollbar">
          <div className="flex items-center gap-3 lg:hidden">
            ...
            <span className="font-black text-slate-700 text-sm uppercase tracking-widest">Admin Overview</span>
          </div>
      ...
      {(activeTab === 'roster' || activeTab === 'approvals') && (
        <div className="flex-1 flex overflow-hidden min-h-0">
          ...
          {/* COLUMN 2: CANDIDATE LIST — hidden on mobile when detail open */}
                  ...
                  filteredCandidates.map(candidate => {
                    ...
                    return (
                      <motion.button
                        ...
                        onClick={() => { setSelectedClientId(candidate.uid || candidate.id); setMobilePanel('detail'); }}
                        className={cn(
          ...
          {/* COLUMN 3: CANDIDATE DOSSIER — hidden on mobile when list shown */}
              ...
              <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="p-4 lg:p-6 border-b border-slate-900 bg-slate-950 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-3 lg:gap-4">
                    <button onClick={() => setMobilePanel('list')} className="lg:hidden p-2 bg-slate-800 rounded-xl text-slate-400 hover:text-white shrink-0">←</button>
                    <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400 font-black text-base lg:text-lg font-mono shrink-0">
                          ...
                          <div className="space-y-6">
                            <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-6">
                              ...
                              <div className="space-y-4">
                                ...
                                <DetailRow label="Date of Birth" value={appData.dob} icon={<Calendar className="w-4 h-4" />} />
                                <DetailRow label="Aadhar Number" value={appData.aadharNumber} icon={<CreditCard className="w-4 h-4" />} />
                                <DetailRow label="PAN Number" value={appData.panNumber} icon={<CreditCard className="w-4 h-4" />} />
