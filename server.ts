...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    try {
      // For migration compatibility: we might have plaintext passwords from PreRegistration if we didn't migrate properly.
      // But we will hash them during import.
      const user = await prisma.user.findUnique({ where: { email: email.toLowerCase() } });
      if (!user) return res.status(400).json({ error: "Invalid credentials" });

      // We need to fetch the password hash. Wait, I didn't add password field to User model!
      // Let's rely on Prisma schema to be updated. (I will update schema.prisma in a moment)
      // Assuming 'passwordHash' is on User
  ...
  app.put("/api/clients/:uid", authenticateToken, async (req: any, res: any) => {
    try {
      ...
      // Sync isApproved on User when client status changes
  ...
  app.get("/api/clients", authenticateToken, async (req: any, res: any) => {
    try {
      ...
      // Fetch matching users to enrich with email/displayName
  ...
  app.post("/api/admin/create-user", authenticateToken, async (req: any, res: any) => {
    ...
    try {
      ...
      const user = await prisma.user.create({
        data: {
          uid, email: email.toLowerCase(), displayName, role, isApproved: true, passwordHash: hashedPassword
        }
      });
      if (role === 'client') {
  ...
  app.post("/api/users/counselor", authenticateToken, async (req: any, res: any) => {
    ...
    try {
      // Find the highest existing counselor ID
      ...
      const newIdNum = maxId > 0 ? maxId + 1 : 3; // fallback to 3 if not found
  ...
  // Scrape a job URL with Puppeteer then extract structured data via Claude
  ...
  // Parse a raw job description text and return structured fields via Claude
